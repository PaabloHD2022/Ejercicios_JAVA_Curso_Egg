...
public class ClienteDAO extends DAO<Cliente> {
    ...
    public List<Cliente> buscarClienteNombre(String nombre) {
        conectarBase();
        List<Cliente> clientes = em.createQuery("SELECT c FROM Cliente c where c.nombre = :nombre").setParameter("nombre", nombre).getResultList();
        desconectarBase();
        return clientes;
    }
    ...
    public List<Cliente> buscarClienteDocumento(long dni) {
        conectarBase();
        List<Cliente> clientes = em.createQuery("SELECT c FROM Cliente c where c.documento = :documento").setParameter("documento", dni).getResultList();
        desconectarBase();
        return clientes;
    }
