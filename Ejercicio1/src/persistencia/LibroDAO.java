...
public class LibroDAO extends DAO<Libro> {
    ...
    public List<Libro> buscarLibroTitulo(String titulo) {
        conectarBase();
        List<Libro> libros = em.createQuery("SELECT Titulo FROM Libro l WHERE l.titulo = :titulo").setParameter("titulo", titulo).getResultList();
        desconectarBase();
        return libros;
    }
    ...
    public List<Libro> buscarLibroAutor(String nombre) {
        conectarBase();
        List<Libro> libros = em.createQuery("SELECT l FROM Libro l WHERE l.autor.id = (SELECT a.id FROM Autor a where a.nombre = :nombre)").setParameter("nombre", nombre).getResultList();
        desconectarBase();
        return libros;
    }
